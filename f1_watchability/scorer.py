...
def _score_close_finish(
    ...
    Two-component close finish score:
      - 40% winner gap: P1→P2 gap at the flag
      - 60% pack tightness: avg gap between consecutive top-10 finishers
    """
    ...
    # Component 1: winner gap (P1→P2)
    p2 = top10[1] if len(top10) > 1 else None
    p2_gap = final_gaps.get(p2.driver_number) if p2 else None
    winner_score = _clamp(100.0 - _linear(p2_gap, 0.0, 30.0)) if p2_gap is not None else 50.0
...
def _score_overtakes(
    ...
    """
    Count net position gains in the top 10 using date-ordered snapshots.
    OpenF1 position data has no lap_number — sorted by date field instead.
    """
    ...
    for num, positions in timeline.items():
        if num not in top10_nums or len(positions) < 2:
            continue
        for i in range(1, len(positions)):
            ...
            if curr_pos < prev_pos:
                ...
                if gain >= 8:  # skip pit exit spikes
                    continue
                overtake_count += gain
...
def _score_safety_car(rc_events: list[RaceControlEvent]) -> FactorScore:
    """SC and VSC deployments — check VSC first to avoid substring collision."""
    deploy_events = [e for e in rc_events if "deploy" in e.message.lower()]
